...
projectName = config.get('projectName', 'gs-project')
account_id = config.get('accountId')
if not account_id:
    session = boto3.Session()
    sts_client = session.client('sts', region_name=bedrock_region)
...
def create_agentcore_memory_role():
    """Create IAM role for Bedrock Memory with required permissions"""
        
    role_name = f"BedrockMemoryRoleFor{projectName.replace('-', '_')}"
    policy_name = f"BedrockMemoryPolicyFor{projectName.replace('-', '_')}"
    
    # Policy document for Bedrock Memory operations
    policy_document = {
        "Version": "2012-10-17",
        "Statement": [
            {
    ...
    try:
        iam_client = boto3.client('iam', region_name=bedrock_region)
        ...
        try:
            ...
            logger.info(f"Existing policy found: {policy_arn}")
            ...
            response = iam_client.create_policy(
                ...
                PolicyDocument=json.dumps(policy_document),
                Description=f"Policy for Bedrock Memory operations for {projectName}"
            )
            policy_arn = response['Policy']['Arn']
            logger.info(f"New policy created: {policy_arn}")
        
        # Create or get role
        try:
            existing_role = iam_client.get_role(RoleName=role_name)
            role_arn = existing_role['Role']['Arn']
            logger.info(f"Existing role found: {role_arn}")
            ...
            response = iam_client.create_role(
                ...
                Description=f"Role for Bedrock Memory operations for {projectName}"
            )
            role_arn = response['Role']['Arn']
            logger.info(f"New role created: {role_arn}")
        ...
        logger.error(f"Error creating agentcore memory role: {e}")
        raise

agentcore_memory_role = config.get('agentcore_memory_role')
if not agentcore_memory_role:
    agentcore_memory_role = create_agentcore_memory_role()
    config['agentcore_memory_role'] = agentcore_memory_role
    with open(config_path, "w", encoding="utf-8") as f:
        json.dump(config, f, ensure_ascii=False, indent=4)
